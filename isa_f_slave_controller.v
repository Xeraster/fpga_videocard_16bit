...
//this is based on the "isa-f" protocol Im emprovising up. It wastes less clock cycles than regular isa and can run at up to 40mhz for a theoretical maximum transfer speed of 80mb/s over the isa bus
//isa-f ALWAYS runs at the bus speed of the host cpu and deals with slower devices with wait states instead of an asynchronous clock. Have a slow device that can only do 8mhz or less? Simple: have it insert wait states.
//this breaks compatibility with actual isa devices in exchange for a theoretical 10x performance boost
module isaSlaveBusController(
    ...
    output IO_RDY,              //set low if to insert wait states, high for no wait states. goes to ISA-F pin #41. the ISA_RDY signal
    ...
    input SMEMR,
    input SMEMW,
    input IOR,                  //for now only io cycles trigger stuff
    input IOW,
    input ISA_CLK,              //The isa bus clock, technically on a different clock domain. 25mhz on the test system. isa-f specification allows for up to 40mhz.
    
    output done,                  //1 if finished. 0 if busy

    output TE0, TE1, TE2, TE3,  //tranciever enable signals for the 74ALVC16245 chips
    output FPGA_WR,
    input FPGACLK,               //the FAST clock running the fpga core, asynchronous from the isa-f bus.
    ...
    assign FPGA_WR = (~IOR/* | ~absMEMR | ~absSMEMR*/);
    ...
    always@(posedge FPGACLK) begin
        ...
        if (lastAdsRequest >= 20'h420 & lastAdsRequest <= 20'h430 /*& ~mio*/ & ((~IOR | ~IOW)) & ~BALE) begin
            actualBusCycle <= 1;
